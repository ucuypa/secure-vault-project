<template>
    <div class="dashboard-layout">
        ...
        <aside class="sidebar-menu">
            ...
            <div class="folder-list-section" v-if="folderList.length > 0">
                <h3 class="section-label">My Folders</h3>
                <div v-for="folder in folderList" :key="folder.id" class="menu-item" @click="openFolder(folder)">
                    <Folder class="icon-small color-folder" />
                    <span class="truncate-text">{{ folder.original_name }}</span>
                </div>
            </div>
        </aside>
        ...
        <main class="main-content">
            ...
            <section class="content-area">
                <div class="content-header">
                    <!-- Tombol Back Hadir Kembali di Sini! -->
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <button @click="goBack" class="btn-icon" style="background: var(--bg-hover);">
                            <ArrowLeft class="icon" />
                        </button>
                        <h1 class="page-title">{{ currentFilter === 'folders' ? 'Folders' : (route.query.name ||
                            'Folder') }}</h1>
                    </div>
                    ...
                    <table class="file-table">
                        ...
                        <tbody>
                            <tr v-for="file in filteredFiles" :key="file.id" class="file-row"
                                @dblclick="openFolder(file)" style="cursor: pointer;">
                                <td>
                                ...
                                <td class="text-muted">{{ file.mime_type === 'directory' ? '--' :
                                    formatSize(file.file_size) }}</td>
                                <td class="text-right">
                            ...
                            <tr v-if="files.length === 0">
                                <td colspan="4" class="empty-state">This folder is empty.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div v-if="showFolderModal" class="modal-overlay" @click.self="closeFolderModal">
            <div class="modal-card">
                ...
                <div class="modal-footer">
                    ...
                    <button @click="submitFolder" :disabled="!newFolderName.trim() || isCreatingFolder"
                        class="btn-confirm">
...
import { useRoute, useRouter } from 'vue-router'; // WAJIB import route dan router
...
import {
    ...
    User, X, ArrowLeft, Bell, Grip, Image as ImageIcon, Users, FileQuestion, Trash2, ChevronRight
...
// Mengambil ID Folder secara dinamis dari URL (misal: /folder/5, maka nilainya 5)
...
const fetchFiles = async () => {
    try {
        const response = await axios.get('/files', {
            params: { parent_id: currentFolderId.value } // Fetch data berdasarkan ID folder ini
        });
...
// Pantau jika pengguna melompat dari satu folder ke folder lain via sidebar
...
const handleFileUpload = async (event) => {
    ...
    // Upload file KE DALAM folder ini
...
const submitFolder = async () => {
    ...
    try {
        await axios.post('/files', {
            ...
            parent_id: currentFolderId.value // Buat folder baru DI DALAM folder ini
        });
...
const folderList = computed(() => { return files.value.filter(file => file.mime_type === 'directory'); });
...
// Masuk ke folder yang lebih dalam (Nested Folder)
...
// Fungsi Kembali (Back) menggunakan sejarah peramban bawaan
const goBack = () => {
    router.back();
};
